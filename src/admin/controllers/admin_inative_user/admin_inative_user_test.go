...
func TestInativeUserWithJwtOfNonAdminUser(t *testing.T) {
	...
	if err != nil {
		log.Fatalf("error to generate jwt in 'TestInativeUserWithJwtOfNonAdminUser' test: %v", err)
	}
...
func TestInativeUserWithSuccess(t *testing.T) {
	...
	if err != nil {
		log.Fatalf("error to generate jwt in 'TestInativeUserWithSuccess' test: %v", err)
	}
	req.Header.Set("Authorization", "Bearer "+*jwtForTest)
	response := httptest.NewRecorder()
	r.ServeHTTP(response, req)
