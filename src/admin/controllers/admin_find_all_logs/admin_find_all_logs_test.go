...
func TestAdminFindAllLogsWithJwtOfNonAdminUser(t *testing.T) {
	...
	if err != nil {
		log.Fatalf("error to generate jwt in 'TestAdminFindAllLogsWithJwtOfNonAdminUser' test: " + err.Error())
	}
...
func TestAdminFindAllLogsWithSuccess(t *testing.T) {
	...
	if err != nil {
		log.Fatalf("error to generate jwt in 'TestAdminFindAllLogsWithSuccess' test: " + err.Error())
	}
	req.Header.Set("Authorization", "Bearer "+*jwtForTest)
	response := httptest.NewRecorder()
	r.ServeHTTP(response, req)
