...
func (r *SQLRepository) FindUser(find string) (*user_entity.User, error) {
	...
	if utils_validators.IsValidEmail(find) {
		err = r.Db.QueryRow("SELECT id, email, is_admin, password FROM users WHERE email = $1", find).Scan(&user.Id, &user.Email, &user.IsAdmin, &user.Password)
	} else {
		err = r.Db.QueryRow("SELECT id, email, is_admin, password FROM users WHERE id = $1", find).Scan(&user.Id, &user.Email, &user.IsAdmin, &user.Password)
	}
	if err != nil {
