...
func (u *ChangePasswordInRecovery) Executar(c *gin.Context, find string, code user_dtos.ChangePasswordInRecoveryRequest) {
	...
	if err != nil {
		...
		go utils_usecases.CreateLog(nil, "users/changePasswordInRecovery/:find", "POST", false, log_messages.JWT_INVALID_SIGNATURE, c.ClientIP())
		return
	}
	...
	if idFinInNumber != idInClaims && find != emailInClaims {
		...
		go utils_usecases.CreateLog(&idInClaimsConvertedToInt, "users/changePasswordInRecovery/:find", "POST", false, log_messages.JWT_UNAUTHORIZED, c.ClientIP())
		return
	}
	...
	if err != nil {
		statusCode := http.StatusUnauthorized
		c.JSON(statusCode, gin.H{"error": err.Error(), "statusCode": statusCode})
		go utils_usecases.CreateLog(&idInClaimsConvertedToInt, "users/changePasswordInRecovery/:find", "POST", false, log_messages.VERIFY_PASSWORD_RECOVERY_CODE_WITHOUT_SUCCESS, c.ClientIP())
		return
	}

	go utils_usecases.CreateLog(&idInClaimsConvertedToInt, "users/changePasswordInRecovery/:find", "POST", true, log_messages.VERIFY_PASSWORD_RECOVERY_CODE_WITH_SUCCESS, c.ClientIP())
