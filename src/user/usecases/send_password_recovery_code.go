...
func (u *SendPasswordRecoveryCode) Executar(c *gin.Context, find string) {
	...
	if err != nil {
		...
		go utils_usecases.CreateLog(nil, "users/sendPasswordRecoveryCode/:find", "POST", false, log_messages.JWT_INVALID_SIGNATURE, c.ClientIP())
		return
	}
	...
	if idFinInNumber != idInClaims && find != emailInClaims {
		...
		go utils_usecases.CreateLog(&idInClaimsConvertedToInt, "users/sendPasswordRecoveryCode/:find", "POST", false, log_messages.JWT_UNAUTHORIZED, c.ClientIP())
		return
	}
	...
	if err != nil {
		...
		c.JSON(statusCode, gin.H{"error": err.Error(), "statusCode": statusCode})
		go utils_usecases.CreateLog(&idInClaimsConvertedToInt, "users/sendPasswordRecoveryCode/:find", "POST", false, log_messages.SEND_PASSWORD_RECOVERY_CODE_WITHOUT_SUCCESS, c.ClientIP())
		return
	}

	//TODO: Send email with de code
	go utils_usecases.CreateLog(&idInClaimsConvertedToInt, "users/sendPasswordRecoveryCode/:find", "POST", true, log_messages.SEND_PASSWORD_RECOVERY_CODE_WITH_SUCCESS, c.ClientIP())
